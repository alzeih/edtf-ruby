...
rule
  ...
  pua_year :
    UA year { result = uoa(Date.new(val[1]), val[0], :year) }
    | year UA { result = uoa(Date.new(val[0]), val[1], :year) }
    | UA year UA { result = uoa(uoa(Date.new(val[1]), val[0], :year), val[2], :year) }
